...
const createNewConfig = () => {
    readDefaultConfig().then((contents) => {
        ...
        for (let i = 0, len = locationsList.length; i < len; i++) {
            ...
            resultData.data[locationName] = Object.assign({keysAvailable: keysLocations.indexOf(locationName) !== -1}, defaultConfigJson.data);
        }
...
const writeResult = (json) => {
    ...
    message += `Норм редких: ${data.Sets}\n`;
    message += `Норм магических: ${data.Magic}\n`;
    message += `Норм баз: ${data.Bases}\n`;
    message += `Норм джевелов: ${data.Jewels}\n\n`;
    message += `Руны:\n`;
